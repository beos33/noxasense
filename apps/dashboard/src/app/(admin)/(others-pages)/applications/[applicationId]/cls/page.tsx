...
export default function CLSDetail({ params }: { params: Promise<{ applicationId: string }> }) {
  ...
  const fetchCLSData = async () => {
    ...
    try {
      ...
      console.error('Error fetching CLS data:', err.message);
      setError(err.message);
    } finally {
      setIsLoading(false);
    }
  ...
  const getCLSRating = (cls: number) => {
    if (cls <= 0.1) return { rating: 'good', color: 'text-green-600', bg: 'bg-green-100' };
    if (cls <= 0.25) return { rating: 'needs-improvement', color: 'text-yellow-600', bg: 'bg-yellow-100' };
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-gray-500">Loading CLS data...</div>
      </div>
    );
  }
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          <PageBreadcrumb pageTitle="CLS Data Not Found" />
          <Button
            size="sm"
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <PageBreadcrumb 
          pageTitle="CLS Performance Data"
          breadcrumbItems={[
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800">
              <div className="text-2xl font-bold text-purple-900 dark:text-purple-100">
                {(pageviews.reduce((sum, p) => sum + p.cls, 0) / pageviews.length).toFixed(3)}
              </div>
              <p className="text-sm text-purple-700 dark:text-purple-300">Average CLS</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
              <div className="text-lg font-semibold text-gray-900 dark:text-white/90">
                {Math.min(...pageviews.map(p => p.cls)).toFixed(3)}
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Best CLS</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
              <div className="text-lg font-semibold text-gray-900 dark:text-white/90">
                {Math.max(...pageviews.map(p => p.cls)).toFixed(3)}
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Worst CLS</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
              <div className="text-lg font-semibold text-gray-900 dark:text-white/90">
                {pageviews.filter(p => p.cls <= 0.1).length}
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Good CLS</p>
            </div>
          </div>

          <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
            <h4 className="font-medium text-purple-800 dark:text-purple-200 mb-2">
              About CLS (Cumulative Layout Shift)
            </h4>
            <p className="text-sm text-purple-700 dark:text-purple-300 mb-3">
              CLS measures the sum total of all individual layout shift scores for every unexpected layout shift 
              that occurs during the entire lifespan of the page.
            </p>
            <div className="text-sm text-purple-600 dark:text-purple-400 space-y-1">
              <p>• <strong>Good:</strong> 0-0.1</p>
              <p>• <strong>Needs Improvement:</strong> 0.1-0.25</p>
              <p>• <strong>Poor:</strong> Over 0.25</p>
            </div>
          </div>
        </div>
      </div>
          ...
          {pageviews.length > 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {pageviews.map((pageview) => {
                    const rating = getCLSRating(pageview.cls);
                    return (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500 dark:text-gray-400">
                No CLS data available for this application.
              </p>
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">
                CLS data will appear here once users start visiting your application.
              </p>
            </div>
          )}
        </div>
